...
The release app declares only the following Android permissions:

| Permission | When requested | Purpose |
|---|---|---|
| `INTERNET` | Granted automatically at install | Make network requests to a user-configured server. |
| `POST_NOTIFICATIONS` (Android 13+) | When the user enables response-completion notifications | Post a notification after a chat response finishes while Hermex is in the background. If denied, notifications are not shown and the rest of the app continues to work. |
| `RECORD_AUDIO` | When the user taps the microphone in the chat composer | Provide voice-to-text input through Android's installed speech-recognition service. |
...
The app permits plain HTTP only for localhost and private-network server addresses.
Public HTTP server addresses are blocked. Local HTTP is supported to make
self-hosting possible — for example, running the Hermes server on a computer on the
...
Hermex requires HTTPS for public server addresses. If the user chooses permitted
local HTTP, the app still transmits only what the user explicitly types, attaches,
or configures — but it transmits it in the clear.
